...
export class RTBJob extends Job {
  ...
  public async run () : Promise< void > {
    await this.protect( async () => {
      ...
      for ( const [ i, raw ] of Object.entries( entries ) ) {
        ...
        const { profile, action } = await ProfileManager.process( uri, id, profileData, 'updateData' ) || {};
      ...
      // --- create "rtb" list ---
      const list = List.get( 'rtb' ) ?? List.create( 'rtb', indexItem( { items } ) );
  ...
  public static readonly cron: TCronJob = [ {
    cronexpr: '30 23 * * *', // run at 11:30 PM on every day
  } ] as const;
}
