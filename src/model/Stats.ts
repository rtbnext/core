...
import type { TListSnapshot, TRTBListItem } from '@rtbnext/schema/src/model/list';
...
export class Stats implements IStats {
  ...
  public generateTop10Entry ( snapshot: TListSnapshot< TRTBListItem > ) : boolean {
    return log.catch( () => {
      ...
      for ( const { uri, name, rank, networth } of snapshot.items.filter( i => i.uri ).slice( 0, 10 ) ) {
        const prevItem = last?.find( i => i.uri === uri );

        top10.push( { uri: uri!, name, rank, networth, flag: ! last ? 'unknown'
